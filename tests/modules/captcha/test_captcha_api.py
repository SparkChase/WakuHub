...
async def test_verify_captcha_api_success(client, redis_client):
    gen = await client.get("/api/v1/captcha")
    key = gen.json()["data"]["key"]
    code = await redis_client.get(f"{settings.CAPTCHA_KEY_PREFIX}{key}")

    resp = await client.post("/api/v1/captcha/verify", json={"key": key, "code": code})
    assert resp.status_code == 200
    body = resp.json()
    assert body["code"] == 200
...
async def test_verify_captcha_api_wrong_code(client):
    ...
    resp = await client.post("/api/v1/captcha/verify", json={"key": key, "code": "0000"})
...
async def test_verify_captcha_api_expired_key(client):
    resp = await client.post(
        "/api/v1/captcha/verify", json={"key": "no-such-key", "code": "abcd"}
    )
    assert resp.status_code == 200
    assert resp.json()["code"] == 400
...
async def test_verify_captcha_api_one_time_use(client, redis_client):
    ...
    first = await client.post("/api/v1/captcha/verify", json={"key": key, "code": code})
    assert first.json()["data"] is True

    # 第二次用同一 key 应失败（已被消费）
    second = await client.post("/api/v1/captcha/verify", json={"key": key, "code": code})
