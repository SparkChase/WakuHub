...
# 用法: cd WakuHub && python scripts/init_mysql.py
# ============================================================
...
# 把 WakuHub/ 加入 Python 路径，让 src.* 可以导入
...
def import_data():
    ...
    # charset 用 utf8mb4，医学文本含大量中文/特殊字符
    ...
    # model 字段：name（唯一）
    # MySQL 无 RETURNING，统一用「先 SELECT 再 INSERT + lastrowid」
    # ----------------------------------------------------------
    logger.info("导入科室数据...")
    dept_set = set()
    ...
    for dept_name in tqdm(dept_set, desc="科室"):
        ...
        if row:
            ...
            dept_name_to_id[dept_name] = cur.lastrowid
    ...
    for symptom_name in tqdm(symptom_set, desc="症状"):
        ...
        if row:
            ...
            cur.execute("INSERT INTO symptoms (name) VALUES (%s)", (symptom_name,))
            symptom_name_to_id[symptom_name] = cur.lastrowid
    ...
    for drug_name in tqdm(drug_set, desc="药品"):
        ...
        if row:
            ...
            cur.execute("INSERT INTO drugs (name) VALUES (%s)", (drug_name,))
            drug_name_to_id[drug_name] = cur.lastrowid
    ...
    for record in tqdm(records, desc="疾病"):
        ...
        # 疾病 name 唯一：已存在则跳过（连带跳过关联，避免重复导入）
