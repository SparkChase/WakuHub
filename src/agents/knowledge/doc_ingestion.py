...
CHUNK_SIZE = 512
CHUNK_OVERLAP = 64
# Milvus VARCHAR max_length=65535 按 UTF-8 字节计，中文 3 字节/字，需按字节截断
MAX_TEXT_BYTES = 65000


def _truncate_bytes(text: str, max_bytes: int = MAX_TEXT_BYTES) -> str:
    """按 UTF-8 字节截断，丢弃末尾被截断的半个多字节字符。"""
...
def _split_markdown(md_text: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP):
    """
    ...
    for i in range(0, len(texts), batch_size):
        ...
        for j, (text_content, emb) in enumerate(zip(batch_texts, embeddings)):
            ...
            all_data.append({
                ...
                "text": _truncate_bytes(text_content),
                "embedding": emb,
            })
...
async def ingest_drug_instructions(
    ...
    for drug_name, full_instruction in rows:
        if not full_instruction or not full_instruction.strip():
            continue
        ...
        texts = splitter.split_text(full_instruction)
        if not texts:
            continue
