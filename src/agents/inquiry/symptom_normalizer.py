...
class SymptomsOutput(BaseModel):
    ...
    symptoms: list[str] = Field(description="从用户描述中提取并标准化后的医学症状术语列表，无症状时为空列表")
...
async def extract_and_normalize_symptoms(user_input: str,llm:BaseChatModel) -> list[str]:
    """
       第一层：LLM 提取 + 标准化。
    ...
    try:
        result: SymptomsOutput = await structured_llm.ainvoke([SystemMessage(content=prompt)])
...
async def match_symptoms_in_neo4j(symptoms: list[str], neo4j_driver: AsyncDriver) -> tuple[list[str],list[str]]:
    """
       第二层：Neo4j 图数据库匹配。
       输入：标准化后的症状列表
       输出：匹配上的症状列表，未匹配上的症状列表
       """
    if not symptoms:
        return [], []
